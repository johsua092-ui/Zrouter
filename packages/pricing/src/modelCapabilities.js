...
export function modelSupportsToolCalling(model) {
    ...
    for (const key of keys) {
        if (key in data) {
            ...
            if (entry && entry.tool_call === false)
                return false;
            if (entry && entry.tool_call === true)
                return true;
        }
    }
    ...
    if (/embed|rerank|image[-_]?gen|tts|stt|whisper|audio/.test(lower))
        return false;
 ...
 */
export function isToolCallingNotSupportedError(err) {
    if (!err)
        return false;
