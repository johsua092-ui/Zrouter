...
export function parseGeminiModelName(rawModel) {
...
export function buildGeminiContents(req) {
    return req.messages.map((m) => ({
        role: m.role === "assistant" ? "model" : "user",
        parts: [{ text: typeof m.content === "string" ? m.content : JSON.stringify(m.content) }]
    }));
}
export function buildGeminiBody(contents, modelName, token) {
    if (token.startsWith("ya29.")) {
        return { model: modelName, request: { contents } };
    }
    return { contents };
}
export function buildGeminiUrl(baseUrl, modelName, token) {
...
export function geminiToOpenAIResponse(data, requestedModel) {
 ...
 */
export function buildIdeRequestId({ body, request, sessionId, model, requestType }) {
...
export function buildGeminiStreamUrl(baseUrl, modelName) {
...
function mergeAllOf(obj) {
    ...
    if (o.allOf && Array.isArray(o.allOf)) {
        ...
        if (merged.required)
            o.required = [...(o.required || []), ...merged.required];
    }
    for (const value of Object.values(o)) {
        if (value && typeof value === "object")
            mergeAllOf(value);
...
function flattenAnyOfOneOf(obj) {
    ...
    if (o.anyOf && Array.isArray(o.anyOf) && o.anyOf.length > 0) {
        const nonNull = o.anyOf.filter((s) => s && s.type !== "null");
        if (nonNull.length > 0) {
    ...
    if (o.oneOf && Array.isArray(o.oneOf) && o.oneOf.length > 0) {
        const nonNull = o.oneOf.filter((s) => s && s.type !== "null");
        if (nonNull.length > 0) {
            const selected = nonNull[selectBest(nonNull)];
            delete o.oneOf;
            Object.assign(o, selected);
...
function flattenTypeArrays(obj) {
    ...
    if (o.type && Array.isArray(o.type)) {
        const nonNullTypes = o.type.filter((t) => t !== "null");
...
function ensureArrayItems(obj) {
    ...
    if (type === "array" && (!o.items || typeof o.items !== "object")) {
        const prefix = Array.isArray(o.prefixItems)
            ? o.prefixItems.find((s) => s && typeof s === "object")
            : undefined;
        const fallback = prefix ?? (o.contains && typeof o.contains === "object" ? o.contains : undefined);
...
function cleanupRequired(obj) {
    ...
    if (o.required && Array.isArray(o.required) && o.properties) {
        ...
        const valid = o.required.filter((f) => Object.prototype.hasOwnProperty.call(props, f));
        if (valid.length === 0)
            delete o.required;
        else
            o.required = valid;
...
export function createGeminiStreamState(model, toolNameMap) {
    return { model, functionIndex: 0, geminiToolCallCount: 0, toolNameMap };
}
function geminiChunkMeta(state) {
    return {
...
function buildGeminiChunk(state, delta, finishReason) {
    return {
        id: `chatcmpl-${state.messageId || Date.now()}`,
...
function emitGeminiFunctionCall(functionCall, state, thoughtSignature) {
 ...
 */
export function geminiStreamToOpenAIChunks(chunk, state) {
    if (!chunk)
        return null;
    ...
    if (content?.parts) {
        for (const part of content.parts) {
            ...
            if (part.text !== undefined && part.text !== "") {
                ...
                if (textualToolCall) {
                    results.push(emitGeminiFunctionCall({
    ...
    if (usageMeta) {
        const promptTokens = Number(usageMeta.promptTokenCount ?? 0);
        const completionTokens = Number(usageMeta.candidatesTokenCount ?? usageMeta.candidates_tokens_count ?? 0);
        const cachedTokens = Number(usageMeta.cachedContentTokenCount ?? usageMeta.totalCachedTokens ?? 0);
        const total = promptTokens + completionTokens;
        state.usage = {
            prompt_tokens: promptTokens,
            completion_tokens: completionTokens,
    ...
    if (Array.isArray(rawCredits)) {
        state.remainingCredits = rawCredits;
    }
    // Finish reason
    if (candidate.finishReason) {
        let finishReason = String(candidate.finishReason) === "STOP"
            ? "stop"
 ...
 */
export function stripBlacklistedRequest(obj) {
...
export function parseImageConfig(model) {
...
export const COMPETITIVE_AGENT_PROMPT_PATTERNS = [
...
export function stripCompetitiveAgentPrompts(text) {
 ...
 */
export function stripTrailingAssistantTurn(contents) {
 ...
 */
export function resolveAntigravityOutputCap(modelId) {
    if (!modelId)
        return 8192;
 ...
 */
export const ANTIGRAVITY_PRO_FALLBACK_CHAINS = {
...
export function getAntigravityModelFallbacks(modelName) {
    const clean = parseAntigravityModelName(modelName);
    const raw = modelName.includes("/") ? (modelName.split("/")[1] ?? modelName) : modelName;
    return (ANTIGRAVITY_PRO_FALLBACK_CHAINS[clean] || ANTIGRAVITY_PRO_FALLBACK_CHAINS[raw] || [clean]);
}
/**
 ...
 */
export function parseRetryFromErrorMessage(errorMessage) {
 ...
 */
export function buildAntigravityContents(req) {
    const rawContents = [];
    ...
    for (const m of req.messages) {
        ...
        if (m.role === "assistant" && Array.isArray(m.tool_calls) && m.tool_calls.length > 0) {
            ...
            for (const tc of m.tool_calls) {
                ...
                const rawTc = tc;
                const rawMsg = m;
                const thoughtSignature = rawTc.thoughtSignature ||
                    rawTc.thought_signature ||
                    rawMsg.thoughtSignature ||
                    rawMsg.thought_signature ||
            ...
            const rawM = m;
            const legacyFunctionCall = rawM.function_call;
            let text = typeof m.content === "string" ? m.content.trim() : "";
            if (text) {
                text = stripCompetitiveAgentPrompts(stripZeroWidth(text));
                if (text)
            ...
            const thoughtSignature = legacyFunctionCall.thoughtSignature ||
                legacyFunctionCall.thought_signature ||
                rawM.thoughtSignature ||
                rawM.thought_signature ||
                "skip_thought_signature_validator";
            parts.push({
                functionCall: { name, args },
                thoughtSignature
            ...
            const rawName = (m.tool_call_id ? toolCallNameMap.get(m.tool_call_id) : undefined) ||
                m.name ||
                m.tool_call_id ||
                "function";
            ...
            try {
                ...
                if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
                    responseObj = stripZeroWidth(parsed);
                }
                else {
 ...
 */
export function buildAntigravityTools(req) {
