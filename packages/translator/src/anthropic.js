...
function mapSystemPrompt(system) {
    if (!system)
        return null;
...
function extractAnthropicBlocks(blocks) {
    const parts = [];
    const toolCalls = [];
    const toolResults = [];
...
function mapAnthropicMessage(msg) {
...
export function AnthropicToOpenAIRequest(req) {
    const messages = [];
...
export function OpenAIToAnthropicResponse(res, originalModel, options = {}) {
...
export async function* OpenAIToAnthropicStream(stream, originalModel, options = {}) {
