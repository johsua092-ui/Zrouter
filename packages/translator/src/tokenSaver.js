...
export function CompressGitStatusOrLog(text) {
...
export function CompressGrepOutput(text) {
    const lines = text.split("\n");
...
export function CompressFileListings(text) {
    const lines = text.split("\n");
    const result = [];
    for (const line of lines) {
        const trimmed = line.trim();
        if (!trimmed || trimmed.startsWith("total "))
            continue;
        const ls_match = trimmed.match(/^([drwxlsStT\-]{10})\s+\d+\s+\S+\s+\S+\s+(\d+)\s+[A-Za-z]{3}\s+\d+\s+[\d:]+\s+(.+)$/);
        if (ls_match) {
...
export function CompressGenericLogs(text) {
    const lines = text.split("\n");
    const result = [];
    let last_line = "";
...
export function CompressSingleToolOutput(text, settings) {
    ...
    else if (settings.compressGrep &&
        /^[^\n:]+:\d+[:-]/m.test(processed) &&
        processed.split("\n").filter((l) => /^[^\n:]+:\d+[:-]/.test(l)).length >= 2) {
        processed = CompressGrepOutput(processed);
    }
    else if (settings.compressFileLists &&
...
export function BuildSystemPromptEnhancements(settings) {
    if (!settings.enabled)
        return "";
    const parts = [];
    if (settings.lazySeniorDev.enabled) {
        if (settings.lazySeniorDev.mode === "strict") {
            parts.push(`[SYSTEM INSTRUCTION: STRICT MINIMALIST SENIOR DEV]\n` +
            ...
            parts.push(`[SYSTEM INSTRUCTION: LAZY SENIOR DEV]\n` +
    ...
    if (settings.compressLlmOutput.enabled) {
        if (settings.compressLlmOutput.mode === "ultra_terse") {
            parts.push(`[SYSTEM INSTRUCTION: ULTRA TERSE / CAVEMAN OUTPUT]\n` +
            ...
            parts.push(`[SYSTEM INSTRUCTION: TERSE OUTPUT MODE]\n` +
...
export function ApplyTokenSaver(request, settings) {
    if (!settings.enabled) {
    ...
    const percentage_saved = original_input_tokens > 0
...
export function PreviewTokenSaver(type, text, settings) {
    ...
    const percentage_saved = original_tokens_estimate > 0
