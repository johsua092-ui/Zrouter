...
export const KNOWN_PROVIDER_MAP = Object.freeze(Object.fromEntries(KNOWN_PROVIDERS.map((Provider) => [Provider.id, Provider])));
const KNOWN_PROVIDER_IDS_DESC = Object.freeze(Object.keys(KNOWN_PROVIDER_MAP).sort((A, B) => B.length - A.length));
...
export function providerBaseId(Id) {
    if (UUID_RE.test(Id))
        return Id;
    return (KNOWN_PROVIDER_IDS_DESC.find((Candidate) => Id === Candidate || Id.startsWith(`${Candidate}_`) || Id.startsWith(`${Candidate}-`)) ??
        Id.split("_")[0]?.split("-")[0] ??
        Id);
}
export function isProviderBaseId(Id, BaseId) {
    return Id === BaseId || Id.startsWith(`${BaseId}_`) || Id.startsWith(`${BaseId}-`);
}
export function providerAlias(BaseId) {
    return KNOWN_PROVIDER_MAP[BaseId]?.alias ?? BaseId;
}
export function providerTypeForAlias(Alias) {
    if (Alias in LEGACY_ALIAS_MAP)
        return LEGACY_ALIAS_MAP[Alias];
    return KNOWN_PROVIDERS.find((P) => P.alias === Alias || P.id === Alias)?.id ?? null;
}
export function getProviderWebsiteUrl(ProviderId, DefaultBaseUrl) {
