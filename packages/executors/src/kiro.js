...
function normalizeStopReason(value) {
    ...
    if (["end_turn", "stop", "stop_sequence"].includes(reason))
        return "end_turn";
    if (["tool_use", "tool_calls"].includes(reason))
        return "tool_use";
    if (["max_tokens", "length"].includes(reason))
        return "max_tokens";
    return null;
}
...
export class KiroExecutor {
    ...
    getOrderedBaseUrls() {
        ...
        const regionalize = (url) => region === "us-east-1"
            ? url
    ...
    buildRequest(req) {
        ...
        if (this.providerSpecificData.profileArn &&
            authMethod !== "api_key" &&
            authMethod !== "idc" &&
    ...
    headers(url) {
        ...
        if (this.providerSpecificData.authMethod === "api_key")
    ...
    async chatCompletion(req) {
        const chunks = [];
        for await (const value of this.chatCompletionStream(req))
            chunks.push(value);
    ...
    async *chatCompletionStream(req) {
        ...
        if (!response.body)
            throw new Error("Kiro Provider Error (502): response body is missing");
        ...
        for await (const frame of iterEventStreamFrames(response.body, MAX_FRAME_BYTES)) {
            ...
            if (type === "assistantResponseEvent" || type === "codeEvent") {
                ...
                for (const rawValue of values) {
                    ...
                    const fragment = typeof value.input === "string"
                        ? value.input
                ...
                stop =
                    normalizeStopReason(payload.stopReason ?? payload.stop_reason ?? metadata.stopReason) ?? stop;
            }
        }
        ...
        yield chunk(responseId, req.model, {}, hadTool || stop === "tool_use"
            ? "tool_calls"
            : stop === "max_tokens"
                ? "length"
                : "stop");
