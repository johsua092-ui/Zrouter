...
import { QODER_CHAT_BASE, QODER_CHAT_BASE_ALT, QODER_CHAT_SIG_PATH, QODER_CLIENT_TYPE, QODER_DATA_POLICY, QODER_IDE_VERSION, QODER_JOB_TOKEN_EXCHANGE_URL, QODER_LOGIN_VERSION, QODER_MACHINE_OS, QODER_MACHINE_TYPE, QODER_MODELS, QODER_MODEL_ALIASES, QODER_RSA_PUBLIC_KEY, QODER_USERINFO_URL } from "@srouter/constants";
import { parseDataLine, streamLines } from "./base.js";
/**
 * Detects if Qwen / Qoder reasoning (thinking) is active on a request.
 */
export function isQwenThinkingActive(req, modelConfig) {
    const raw = req;
    const thinking = raw.thinking;
    if (thinking === true || raw.enable_thinking === true)
        return true;
 ...
 */
export function sanitizeQwenThinkingToolChoice(payload, isThinking) {
...
function rsaEncryptBase64(data) {
    const encrypted = crypto.publicEncrypt({ key: QODER_RSA_PUBLIC_KEY, padding: crypto.constants.RSA_PKCS1_PADDING }, Buffer.from(data, "utf8"));
    return encrypted.toString("base64");
}
...
export async function exchangeJobToken(pat) {
...
export async function fetchUserIdForJobToken(jobToken) {
    try {
...
export async function resolvePatCredential(pat) {
...
export class QoderExecutor {
    ...
    async resolveCredentials() {
        ...
        if (!userId && activeToken) {
            ...
            if (userId)
                this.providerSpecificData.userId = userId;
        }
        if (!userId) {
    ...
    async buildPayload(req, creds) {
        ...
        if (!modelConfig) {
            modelConfig = {
                key: qoderKey,
                is_reasoning: qoderKey.includes("reasoning") ||
    ...
    async *chatCompletionStream(req) {
        ...
        for await (const line of streamLines(res.body)) {
            ...
            if (statusVal !== 200) {
                ...
                try {
                    const parsedErr = JSON.parse(inner);
