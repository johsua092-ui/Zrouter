...
function parseRetryFromErrorMessage(errorMessage) {
...
function extractErrorMessage(errorJson, bodyText = "") {
    const parts = [];
    const err = errorJson;
 ...
 */
export async function computeRetryDelay(response, attempt) {
    let bodyText = "";
    let errorJson = null;
    ...
    if (retryMs)
        return retryMs <= MAX_RETRY_AFTER_MS ? retryMs : null;
    if (!isTransientError(response.status, errorMessage))
        return null;
 ...
 */
export async function fetchWithRetry(url, body, headers, maxAttempts = 3) {
