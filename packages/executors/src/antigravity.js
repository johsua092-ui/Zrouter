import { ANTIGRAVITY_BASE_URL, ANTIGRAVITY_IDE_BASE_URL, ANTIGRAVITY_MODELS } from "@srouter/constants";
import { ANTIGRAVITY_IDE_USER_AGENT, accumulateChunks, buildAntigravityContents, buildAntigravityEnvelope, buildAntigravityTools, createGeminiStreamState, generateProjectId, generateSessionId, geminiStreamToOpenAIChunks, getAntigravityModelFallbacks, isImageModel, parseAntigravityModelName, parseImageConfig, parseRetryFromErrorMessage, resolveAntigravityOutputCap, stripBlacklistedRequest } from "@srouter/translator";
...
export class AntigravityExecutor {
    ...
    isLocalProxy() {
        return (/^https?:\/\/127\.0\.0\.1(:\d+)?(\/|$)/.test(this.baseUrl) ||
     ...
     */
    buildRequest(model, req, stream, enabledCreditTypes) {
    ...
    async *streamCandidateWithCreditRetry(model, req) {
        const useCreditsFirst = this.creditsMode === "always" ||
        ...
        try {
            ...
            const shouldRetryCredits = (is429 || isQuotaExhausted) &&
    ...
    async *executeStreamAttempt(model, req, enabledCreditTypes) {
