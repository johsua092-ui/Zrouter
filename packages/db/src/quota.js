...
function formatResetIn(resetTimeStr) {
    if (!resetTimeStr)
        return "24h 0m";
...
export async function fetchAntigravityLiveQuota(providerId, accountName, accessToken, enabled = true) {
    ...
    const Quotas = Object.entries(Data.models).map(([modelId, item]) => {
...
export async function fetchCodeBuddyCNLiveQuota(providerId, accountName, accessToken, enabled = true) {
    if (!accessToken) {
    ...
    const toItem = (name, used, total, resetTime) => {
    ...
    for (const acc of Refills) {
        ...
        const Base = Number.isFinite(CycleDays) && CycleDays <= 1.5
            ? "Daily"
        ...
        Quotas.push(toItem(Name, parseNum(acc.CycleCapacityUsedPrecise, acc.CycleCapacityUsed), parseNum(acc.CycleCapacitySizePrecise, acc.CycleCapacitySize), acc.CycleEndTime));
    }
    Bonuses.forEach((acc, i) => {
        Quotas.push(toItem(`Bonus Pack ${i + 1}`, parseNum(acc.CapacityUsedPrecise, acc.CapacityUsed), parseNum(acc.CapacitySizePrecise, acc.CapacitySize), acc.CycleEndTime));
    });
...
export async function getProviderQuotaAccount(p) {
    const IsAntigravity = isProviderBaseId(p.providerId, "antigravity") || isProviderBaseId(p.id, "antigravity");
    const IsOpenAICodex = isProviderBaseId(p.providerId, "openai_codex") || isProviderBaseId(p.id, "openai_codex");
    const IsOpenAI = isProviderBaseId(p.providerId, "openai") || isProviderBaseId(p.id, "openai");
    const IsAnthropic = isProviderBaseId(p.providerId, "anthropic") || isProviderBaseId(p.id, "anthropic");
    const IsCodeBuddyCN = isProviderBaseId(p.providerId, "codebuddy-cn") || isProviderBaseId(p.id, "codebuddy-cn");
    const IsCodeBuddy = !IsCodeBuddyCN &&
    ...
    const UsageMetrics = UsageRows.map((row) => ({
        model: row.model,
...
export async function getQuotaSummaryDB() {
